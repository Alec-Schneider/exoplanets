...
class ExoplanetDataset(Dataset):
    ...
    def __init__(self, root, train=True):
        ...
        if self.train:
            ...
            self.y_train = pd.read_csv(os.path.join(self.root, self.y_train_file))[
                ["LABEL"]
            ]
            ...
            for i in range(len(x_pos)):
                for r in range(num_rotations):
                    ...
                    self.x_train = np.vstack(
                        [self.x_train, rotated_row[np.newaxis, :, :]]
                    )
            ...
            self.y_test = pd.read_csv(os.path.join(self.root, self.y_test_file))[
                "LABEL"
            ].values
    ...
    def load_features(self, file):
        ...
        features = loaded_arr.reshape(loaded_arr.shape[0], loaded_arr.shape[1] // 2, 2)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model-name", type=str, default="torch_model", help="Name to save model as."
    )
